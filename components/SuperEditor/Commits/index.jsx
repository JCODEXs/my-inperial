...
class TrackState {
  ...
  applyTransform(transform) {
    ...
    // Create a new state—since these are part of the view state, a
...
const commit = () => {
  const e = document.querySelector('#commit');
  //   e.preventDefault();
  doCommit(e.elements.message.value || 'Unnamed');
    ...
    let node = elt(
      ...
      commitID != null
        ? elt(
            ...
            null,
            'It was: ',
            elt('strong', null, commit ? commit.message : 'Uncommitted')
          )
        : 'No commit found'
    );
  ...
  return (
    <div>
      <form
        ...
        id="commit"
        // className="ProseMirror-menubar"
      >
        COMMIT:
        <input
          style={{ background: 'rgb(10,10,10)', flex: 1 }}
          type="text"
          id="message"
          name="message"
        />
        <div onClick={commit} id="commitbutton" type="submit">
          commit
        </div>
        <div type="button" id="blame">
          blame at cursor
        </div>
      </form>
      <div id="commits" style={{ marginBottom: '23px' }}></div>
    </div>
  );
};
